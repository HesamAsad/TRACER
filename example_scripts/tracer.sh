...
for sd in 0.0
do
  for alpha_crd in 0.0
  do
    for alpha_cross_kd in 0.0
    do
      for alpha_icl in 0.0
      do
        for alpha_fd in 0.0
        do
          python src/main.py \
            ...
            --csv-img-key filepath --csv-caption-key title --exp_name ImageNet/${method} --cross_fnorm 0.0 \
            --distil_coef ${sd} --l_orth_wv 0 --max_grad_norm 0 --grad_norm_multiplier 0 --warmup_length 500 \
            --wb_project clip_finetune --method ${method} --use_fp16 ${fp16} --run 100 --ema_up_freq ${ema_up_freq} \
