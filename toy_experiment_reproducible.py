...
class ColoredMNISTMultiModal(Dataset):
    ...
    def apply_color(self, image, color):
        ...
        if image.shape[0] == 3:  # Grayscale
            # Create RGB image
            rgb_image = image.clone()
            
            # Apply color tint based on specified color
            if color == 'red':
                rgb_image[0] = image[0] * 0.8  # Red channel
                rgb_image[1] = image[1] * 0.2  # Green channel
                rgb_image[2] = image[2] * 0.2  # Blue channel
            elif color == 'blue':
                rgb_image[0] = image[0] * 0.2  # Red channel
                rgb_image[1] = image[1] * 0.2  # Green channel
                rgb_image[2] = image[2] * 0.8  # Blue channel
...
def finetune_direct(model, train_loader, val_loader, epochs=1, checkpoint_dir="toy_exp_ckpts"):
...
def finetune_l2_regularization(model, train_loader, val_loader, epochs=2, lambda_reg=1e-3, checkpoint_dir="toy_exp_ckpts"):
...
def finetune_self_distillation(model, train_loader, val_loader, epochs=10, temperature=0.2, checkpoint_dir="toy_exp_ckpts"):
...
def finetune_dynamic_self_distillation(model, train_loader, val_loader, epochs=10, 
                                      temperature=1.1, ema_decay=0.99999, checkpoint_dir="toy_exp_ckpts"):
...
if __name__ == "__main__":
    ...
    model_direct = finetune_direct(model, train_colored_loader, val_colored_loader, epochs=20, checkpoint_dir=checkpoint_dir)
    ...
    print("\nStrategy 2: L2 Regularization")
    model_l2 = finetune_l2_regularization(model, train_colored_loader, val_colored_loader, epochs=2, checkpoint_dir=checkpoint_dir)
    ...
    # ==================== Save CSV for analysis script ====================
    # Build a DataFrame that matches the columns expected by toy_experiment_repr_analyze.py
    rows = []
    name_to_res = {
        'Pre-trained': pretrain_acc,
        'Direct FT': direct_acc,
        'L2 Reg': l2_acc,
        'Static Distill': distill_acc,
        'Dynamic Distill': dynamic_acc,
    }
    for name, res in name_to_res.items():
        rows.append({
            'Method': name,
            'Image_Train_Original': float(res['train_img']),
            'Image_Val_Original': float(res['val_img']),
            'Image_Test_Original': float(res['test_img']),
            'Image_Test_Colored': float(res['test_colored_img']),
        })

    results_df = pd.DataFrame(rows, columns=[
        'Method', 'Image_Train_Original', 'Image_Val_Original', 'Image_Test_Original', 'Image_Test_Colored'
    ])
    csv_path = os.path.join(os.getcwd(), 'experiment_results.csv')
    results_df.to_csv(csv_path, index=False)
    print(f"\nSaved experiment results to CSV: {csv_path}")
